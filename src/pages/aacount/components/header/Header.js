...
import header from "../../../../shared/infoHeader/infoHeader.module.css";
import { Link } from "react-router-dom";
import { connect } from "react-redux";

function Header({ activeStore }) {
  return (
    <div className={style.wrapper}>
      <Link to="/" className={`${header.back} ${style.back}`}>
        <svg
          width="12"
          height="12"
          viewBox="0 0 12 12"
      ...
      <p className={style.page_name}>Личный кабинет</p>
    </div>
  );
}
