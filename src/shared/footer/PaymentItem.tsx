...
import { Counter, CounterWrapper, Item, Price } from "./FooterStyle";
...
const PaymentItem: React.FC<IPaymentItem> = ({
  ...
  return (
    <Item id={id} onClick={onClick}>
      ...
      <p>{title}</p>
      {activeStore.length !== 0 && order !== undefined ? (
        <Price>
          <CounterWrapper>
            <Counter>{quantityProds(order)}</Counter>
          </CounterWrapper>
          <div>{sumPrice(order)} ₽</div>
        </Price>
      ) : null}
    </Item>
  );
...
const mapStateToProps = ({ main, stores }: RootState) => {
  return {
